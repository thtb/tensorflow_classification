...
# 共有参数
tf.flags.DEFINE_string("model_type", None, "mode type: fasttext, rnn, cnn")
...
tf.flags.DEFINE_string("use_attention", False, "Whether to use attention")
...
def rnn_estimator(model_dir):
    ...
    def model_fn(features, labels, mode, params):
        ...
        word_embeddings = tf.nn.embedding_lookup(word_embedding_table, word_ids)
        ...
        if FLAGS.use_attention:
            input_layer = attention(text_embedding, FLAGS.attention_dimension)
        else:
            input_layer = tf.reduce_mean(text_embedding, axis=-2)
...
def fasttext_estimator(model_dir):
    ...
    def model_fn(features, labels, mode, params):
        ...
        print(tf.nn.embedding_lookup(
            text_embedding_w, text_ids).shape)
...
def train():
    ...
    if FLAGS.model_type == "rnn":
        ...
        input_fn = inputs.dataset_nn_input_fn
    else:
        estimator = fasttext_estimator(FLAGS.model_dir)
        input_fn = inputs.fasttext_input_fn
    for i in xrange(0, FLAGS.num_epochs):
        train_input = input_fn(tf.estimator.ModeKeys.TRAIN,
                                               FLAGS.train_records,
                                               FLAGS.batch_size)
